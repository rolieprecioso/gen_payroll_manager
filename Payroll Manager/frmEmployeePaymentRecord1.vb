...
Public Class frmEmployeePaymentRecord1
    ...
    Sub fillcombo()
        ...
        Try
            ...
            adp.SelectCommand = New SqlCommand("SELECT distinct  (employeename) FROM employeeRegistration,EmployeePayment where EmployeeRegistration.EmployeeID=Employeepayment.EmployeeID", CN)
            ds = New DataSet("ds")
    ...
    Private Sub button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles button4.Click
        Try
            con = MyConnection
            'con.Open()
            cmd = New SqlCommand("select (EmployeeRegistration.employeeid) as [Employee ID],(employeename) as [Employee Name],(Designation) as [Designation],(Department) as [Department],sum(EmployeePayment.salary) as [Basic Salary],sum(Deduction) as [Deduction],sum(OvertimeAmount) as [Overtime Amount],sum(netpay) as [Net Pay] from employeepayment,EmployeeRegistration where EmployeeRegistration.EmployeeID=EmployeePayment.EmployeeID and paymentdate between " & DateFrom.Value & " & DateTo.Value group by EmployeeRegistration.employeeid,employeename,designation,department order by EmployeeName", con)
